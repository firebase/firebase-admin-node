 ...
 * the namespace of an exported type. This is determined based on barrel
 * exports for services supporting auto-generated types. For example:
 * {
...
function generateFilenameIndex() {
  ...
  serviceFolders.map((serviceName) => {
    if (fs.existsSync(`${repoPath}/src/${serviceName}/index.ts`)) {
      // For example, we would extract InstanceId as below:
      // export interface InstanceId extends instanceIdApi.InstanceId {}
      //                  ^--------^
      const re = /\s*export\s(interface|class|import|const)\s(\w*)/g;
      const data = fs.readFileSync(`${repoPath}/src/${serviceName}/index.ts`, 'utf-8')
                     .toString();
      let results;
      do {
        ...
        if (results) {
          ...
          // For example: remote-config => remoteConfig
          let serviceNameNormalized = serviceName.split('-').reduce((cum, curr) => {
            if (cum === undefined) {
              return curr;
            }
            return `${cum}${curr[0].toUpperCase()}${curr.substr(1)}`;
          });
          typeServiceMap[typeName] = serviceNameNormalized;
        }
      } while (results);
    }
  });
  return typeServiceMap;
}
...
function renameUnmappedFiles() {
  ...
  const filenameIndex = generateFilenameIndex();
