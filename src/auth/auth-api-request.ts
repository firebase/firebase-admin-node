...
function validateAuthFactorInfo(request: AuthFactorInfo): void {
  ...
  else if (typeof request.sharedSecretKey !== 'undefined') {
    //sharedSecretKey should be a valid non-empty string
    if (!validator.isNonEmptyString(request.sharedSecretKey)) {
      throw new FirebaseAuthError(
        AuthClientErrorCode.INVALID_SHARED_SECRET_KEY,
        `The second factor "sharedSecretKey" for "${authFactorInfoIdentifier}" must be a valid non-empty string.`
      )
    }
  }
  else {
