...
import { RulesetMetadataList, RulesetMetadata, RulesFile, SecurityRules } from './security-rules';
 ...
 */
export class SecurityRulesImpl implements SecurityRules, FirebaseServiceInterface {
  ...
  public getFirestoreRuleset(): Promise<Ruleset> {
    return this.getRulesetForRelease(SecurityRulesImpl.CLOUD_FIRESTORE);
  }
  ...
  public releaseFirestoreRuleset(ruleset: string | RulesetMetadata): Promise<void> {
    return this.releaseRuleset(ruleset, SecurityRulesImpl.CLOUD_FIRESTORE);
  }
  ...
  public getStorageRuleset(bucket?: string): Promise<Ruleset> {
    return Promise.resolve()
      ...
      .then((bucketName) => {
        return this.getRulesetForRelease(`${SecurityRulesImpl.FIREBASE_STORAGE}/${bucketName}`);
      });
  }
  ...
  public releaseStorageRuleset(ruleset: string | RulesetMetadata, bucket?: string): Promise<void> {
    return Promise.resolve()
      ...
      .then((bucketName) => {
        return this.releaseRuleset(ruleset, `${SecurityRulesImpl.FIREBASE_STORAGE}/${bucketName}`);
      });
  }
